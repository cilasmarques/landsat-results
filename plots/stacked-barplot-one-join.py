#!/usr/bin/env python3
"""
Script para gerar stacked barplot vertical com três fases:
...
def prepare_three_phase_data(macrogroup_data):
    ...
    # Combinar fases de leitura (E_READ + E_READ_GPU)
    read_rows = []
    for (strategy, algorithm), group in df.groupby(['strategy_plot', 'algorithm']):
        read_data = group[group['PHASE'].isin(['E_READ', 'E_READ_GPU'])]
        if not read_data.empty:
            total_read_time = read_data['TIMESTAMP_median'].sum()
            read_row = read_data.iloc[0].copy()
            read_row['PHASE'] = 'E_READ_COMBINED'
            read_row['TIMESTAMP_median'] = total_read_time
            read_rows.append(read_row)
    
    # Combinar fases de escrita (E_WRITE + E_WRITE_GPU)
    write_rows = []
    for (strategy, algorithm), group in df.groupby(['strategy_plot', 'algorithm']):
        write_data = group[group['PHASE'].isin(['E_WRITE', 'E_WRITE_GPU'])]
        if not write_data.empty:
            total_write_time = write_data['TIMESTAMP_median'].sum()
            write_row = write_data.iloc[0].copy()
            write_row['PHASE'] = 'E_WRITE_COMBINED'
            write_row['TIMESTAMP_median'] = total_write_time
            write_rows.append(write_row)
    
    # Adicionar as novas linhas de leitura e escrita combinadas
    if read_rows:
        df = pd.concat([df, pd.DataFrame(read_rows)], ignore_index=True)
    if write_rows:
        df = pd.concat([df, pd.DataFrame(write_rows)], ignore_index=True)
    
    # Filtrar apenas as fases que queremos mostrar
    phases_to_keep = ['E_READ_COMBINED', 'E_CALCULO', 'E_WRITE_COMBINED']
    df = df[df['PHASE'].isin(phases_to_keep)]
    
    # Mapeamento das etapas para português
    phase_mapping = {
        'E_READ_COMBINED': 'Leitura dos dados\nde entrada',
        'E_CALCULO': 'Processamento',
        'E_WRITE_COMBINED': 'Escrita dos dados\nde saída'
    }
...
def create_one_join_stacked_barplot_clean(macrogroup_data, output_dir):
    """Cria stacked barplot vertical SEM coord_flip e SEM informações dentro das barras"""
...
def main():
    ...
    # print("\n=== GERANDO STACKED BARPLOT VERTICAL (BARRAS VERTICAIS) ===")
    # create_one_join_stacked_barplot_no_flip(macrogroup_data, output_dir)
    
    print("\n=== GERANDO STACKED BARPLOT VERTICAL (BARRAS VERTICAIS LIMPAS) ===")
    create_one_join_stacked_barplot_clean(macrogroup_data, output_dir)
